...
def make_donut(input_response, input_text, input_color):
  ...
  plot = alt.Chart(source).mark_arc(innerRadius=40, cornerRadius=25).encode(
      theta="% value",
  ...
  text = plot.mark_text(align='center', color="#29b5e8", font="Lato", fontSize=34, fontWeight=700, fontStyle="italic").encode(text=alt.value(f'{input_response} %'))
  plot_bg = alt.Chart(source_bg).mark_arc(innerRadius=40, cornerRadius=20).encode(
      theta="% value",
      color= alt.Color("Topic:N",
                      scale=alt.Scale(
                          # domain=['A', 'B'],
